...
- `promote.yml` takes the builds to production by hand (`workflow_dispatch`, from `main` only, inputs checked first):
  Play internal testing → production as a staged rollout raised in place on later runs (`store/play-promote.mjs`, the
  Android Publisher API in one edit that keeps the completed release and never cancels a review), and the newest
  TestFlight build of the version → an App Store version submitted to review with What's New (`store/asc-submit.mjs`,
  the App Store Connect API); both scripts have a dry run and tests against a fake API (`docs/DELIVERY.md`,
  "Promotion").
