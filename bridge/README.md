...
Upgrades arrive by themselves: `setup` installs a daily `remotly-bridge-update.timer` that runs `remotly-bridge update`
(nothing when current; otherwise that release's installer; a bridge that does not stay up gets the previous copy back
when that copy can be verified, otherwise the output says what to do by hand — see `update` below). Re-running the same line, or `remotly-bridge update`, does one now; config and devices stay. LAN mode for a host without Tailscale:
`curl -fsSL https://remotly.dev/install.sh | sh -s -- --lan`.

From a checkout (development): `npm ci --omit=dev`, then `node src/main.ts setup` does the
...
| `status` | Daemon version, herdr connectivity, listener, certificate mode/expiry, devices, push readiness per platform (`direct` / `relay` / off), connected clients. |
...
| `update` | Installs the newest release when this is not it: reads the tag `/releases/latest` points at and, when it is newer than the installed version — or the same, but the running daemon reports another one (an earlier update stopped at a failing check) or none (a daemon from before 0.2.0) — runs that release's own `install.sh` pinned to it with `--no-pair --no-wait --keep-mode --keep-stopped`, told this install's home, launcher directory (`REMOTLY_BIN_DIR`, or the launcher found on PATH), runtime (`REMOTLY_NODE`, unless it is the private Node under `<home>/node`, which the installer refreshes) and release base (`REMOTLY_RELEASE_URL`; `REMOTLY_NODE_DIST` is inherited); the installer keeps the previous copy in `app.prev`. One run per install at a time: a kernel lock on `<home>/update.lock` (`flock -n`; a run that finds it taken says so and exits 0), handed to the installer it starts as an open descriptor — which confirms with `flock -n` on it that it is the lock — and taken by a hand-run `install.sh` as well, so a by-hand install and the timer never overlap; a pending record (`<home>/update-pending.json`) is written before the installer starts and cleared once the outcome is known, so a run that was stopped half-way is finished by the next one; a stop between the two renames of a swap leaves no `app/` at all, which `<home>/repair-app.sh` (written by `setup`, run by both units before they start) undoes from `app.prev` (and a private `node/` from `node.old`) — unless an install holds the lock, since it is between its renames on purpose. Done means the new daemon answered with the new version, from one pid that is the unit's main pid, for 30 s (given up to 10 s to answer at all first). Otherwise, best effort: nothing to undo when the files were not replaced (the unit is restarted if it is not up any more — unless it was stopped meanwhile, which stands); a bridge still answering with the old version is left running (the new copy stays installed; the next run sets it up again); in every other case the previous copy goes back and the unit is restarted on it (`app.failed` keeps the bad one; a unit stopped meanwhile stays stopped) — provided `app.prev` holds the version that ran before, else the message prints the by-hand line: that release's installer with this install's settings (home, release base, launcher directory, runtime, unit, herdr selection, config dir). Nothing when current, when `latest` is older (a withdrawn release is never installed by itself), or while the bridge is stopped (`inactive`: an update starts the bridge; the run says so and exits 0 — a `failed` unit, a crash loop that hit its start-rate limit, is updated, since the update may be what repairs it; no state at all is an error; a stop after that check stands too: the installer's `setup --keep-stopped` leaves the unit stopped, the previous copy goes back, and no restart happens on a unit whose state systemd does not tell); exit 2 on a repository checkout, which has its own delivery. `setup` installs `remotly-bridge-update.timer` (daily, `Persistent=true`, up to an hour of jitter) that runs it as `remotly-bridge-update.service` with the bridge's environment and the installer settings above (mirror, launcher directory, runtime), so an unattended run installs the way the first one did; `setup --no-auto-update`, or `systemctl --user disable --now remotly-bridge-update.timer`, turns that off and later setups respect it, as they do a masked timer or update service. |
