...
# From the heading (exclusive) to the next heading (exclusive); leading lines without visible text dropped, $(…) drops
# the trailing newlines, and a section with no visible text at all is empty.
notes=$(awk -v h="$heading" '
  !on { if ($0 == h || index($0, h " ") == 1) on = 1; next }
  /^#{1,3} / { exit }
  { print }' "$file" | sed '/[^[:space:]]/,$!d')
printf '%s' "$notes" | grep -q '[^[:space:]]' || { echo "the '$heading' section of $file is empty" >&2; exit 1; }
printf '%s\n' "$notes"
