...
/**
 * A fake App Store Connect: `versions`, `submissions`, `items`, `attached` (the build of a READY_FOR_REVIEW version) and
 * `whatsNew` (by version id) are the current state; every call is recorded. Sparse fieldsets are honoured the way the
 * real API does: an item names its version only when `appStoreVersion` is in its fields and included.
 */
function fakeASC({ versions = [shipped], submissions = [], items = [], attached, whatsNew = {}, fail = [] } = {}) {
  const calls = [];
...
const open = { submissions: [{ id: 'subOpen', attributes: { state: 'READY_FOR_REVIEW' } }], items: [{ id: 'i1', attributes: { state: 'READY_FOR_REVIEW' }, relationships: { appStoreVersion: { data: { type: 'appStoreVersions', id: 'v1' } } } }] };
...
test('an update as a new version: created, the newest build of that version attached, What\'s New set, one submission created and submitted', async () => {
  ...
  assert.deepEqual(asc.trail(), [...LOOKUP, 'GET /builds', 'POST /appStoreVersions', 'PATCH /appStoreVersions/vNew/relationships/build',
    'GET /appStoreVersions/vNew/appStoreVersionLocalizations', 'PATCH /appStoreVersionLocalizations/loc-en-vNew', 'GET /apps/app1/reviewSubmissions', 'POST /reviewSubmissions', 'GET /reviewSubmissions/subNew/items', 'POST /reviewSubmissionItems', 'PATCH /reviewSubmissions/subNew']);
  ...
  assert.match(asc.calls[11].path, /fields\[reviewSubmissionItems\]=state,appStoreVersion&include=appStoreVersion/);
...
test('an editable version with the string and its What\'s New is reused; a different release type is patched; an open submission holding it is reused', async () => {
  const asc = fakeASC({ versions: [shipped, v('v1', '0.1.1', 'DEVELOPER_REJECTED')], ...open, submissions: [{ id: 'subOpen', attributes: { state: 'UNRESOLVED_ISSUES' } }], whatsNew: { v1: 'Already there' } });
  const logs = [];
  ...
  assert.deepEqual(asc.trail(), [...LOOKUP, 'GET /builds', 'GET /appStoreVersions/v1/appStoreVersionLocalizations', 'PATCH /appStoreVersions/v1', 'PATCH /appStoreVersions/v1/relationships/build', 'GET /apps/app1/reviewSubmissions', 'GET /reviewSubmissions/subOpen/items', 'PATCH /reviewSubmissions/subOpen']);
  ...
  assert.match(logs[0], /version 0\.1\.1 \(DEVELOPER_REJECTED\) ← build 35 .*What's New as it is/);
});
...
test('a version left READY_FOR_REVIEW by an earlier run is submitted as it stands, after every check', async () => {
  ...
  // No open submission holding it: refused with advice (also in a dry run).
  ...
  const otherItem = fakeASC({ ...state, items: [{ id: 'i9', attributes: { state: 'READY_FOR_REVIEW' }, relationships: { appStoreVersion: { data: { type: 'appStoreVersions', id: 'v9' } } } }] });
  await assert.rejects(submit({ key, bundleId: 'b', version: '0.1.1', fetchFn: otherItem.fetchFn, log: () => {} }), /no open review submission holds version 0\.1\.1/);
});
