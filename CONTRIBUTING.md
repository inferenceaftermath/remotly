...
relay's Worker) and `promote.yml` (production, by hand) run only in the upstream repository, which holds the store and
Cloudflare credentials; bridge releases come from `release.yml` when a bump of `bridge/package.json` merges
(`bridge/scripts/release-prep.sh` prepares the pull request; never push a `bridge-v*` tag by hand; `bridge/README.md`,
"Releases").
