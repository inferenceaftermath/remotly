#!/usr/bin/env bash
# The delivery marker of a lane, refs/delivered/<lane> on origin: the commit whose bundle, build or Worker last reached
# Play, TestFlight or Cloudflare — what ci/plan.sh diffs that lane from next time.
...
set -euo pipefail
usage() { echo "usage: ci/mark-delivered.sh forget|record android|ios|relay" >&2; exit 2; }
[ $# -eq 2 ] || usage
op=$1 lane=$2
case "$lane" in android|ios|relay) ;; *) usage ;; esac
