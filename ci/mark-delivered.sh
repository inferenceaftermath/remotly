#!/usr/bin/env bash
# The delivery marker of a lane, refs/delivered/<lane> on origin: the commit whose bundle, build or code last reached
# Play, TestFlight or the host — what ci/plan.sh diffs that lane from next time.
...
set -euo pipefail
usage() { echo "usage: ci/mark-delivered.sh forget|record android|ios|bridge" >&2; exit 2; }
[ $# -eq 2 ] || usage
op=$1 lane=$2
case "$lane" in android|ios|bridge) ;; *) usage ;; esac
