#!/usr/bin/env bash
# ci/mark-delivered.sh against a throwaway origin with all three markers seeded: `record` moves one lane's marker to HEAD
...
git push -q origin "$one:refs/delivered/android" "$two:refs/delivered/ios" "$one:refs/delivered/bridge"
mark record bridge > "$tmp/log"
check "record: bridge at HEAD, the others as they were" "android=$one ios=$two bridge=$three" "$(markers)"
grep -q "refs/delivered/bridge → $three" "$tmp/log" || fail "no confirmation: $(cat "$tmp/log")"
git checkout -q "$one"
mark record bridge > /dev/null
check "record moves backwards too (it states what was delivered)" "android=$one ios=$two bridge=$one" "$(markers)"
git checkout -q --orphan other && git commit -q --allow-empty -m other && other="$(git rev-parse HEAD)"
mark record ios > /dev/null
check "record onto a rewritten history" "android=$one ios=$other bridge=$one" "$(markers)"
mark forget ios > "$tmp/log"
check "forget: the ios marker is gone, the others stay" "android=$one ios= bridge=$one" "$(markers)"
grep -q "refs/delivered/ios forgotten" "$tmp/log" || fail "no confirmation: $(cat "$tmp/log")"
mark forget ios > "$tmp/log"
check "forget without a marker is fine" "android=$one ios= bridge=$one" "$(markers)"
grep -q "no marker yet" "$tmp/log" || fail "no note: $(cat "$tmp/log")"
mark record ios > /dev/null
check "record after forget" "android=$one ios=$other bridge=$one" "$(markers)"
for bad in "record relay" "start ios" "record" "ios" ""; do
