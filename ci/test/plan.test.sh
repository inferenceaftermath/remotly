#!/usr/bin/env bash
# Exercises ci/plan.sh against a throwaway repository: which paths deliver which lane (android, ios, relay); that
# documentation, paperwork, the bridge (released by a tag, never delivered from main) and the pull-request workflow and
# tests deliver nothing; that a file moved across lanes delivers both; that an unclassified path, a pipeline change or
# an unusable base delivers everything; that workflow_dispatch takes its checkboxes. Run: bash ci/test/plan.test.sh
set -euo pipefail
...
# expect <case> <android> <ios> <relay> <path>...: one commit touching the paths, at main's tip, every lane last
# delivered at its parent.
expect() {
  local name="$1" a="$2" i="$3" r="$4"; shift 4
  commit "$name" "$@"
  tip HEAD; for l in android ios relay; do deliver "$l" HEAD~1; done
  check "$name" "android=$a ios=$i relay=$r build_number=7" "$(plan)"
}
...
expect "pull-request checks and tests"  false false false .github/workflows/ci.yml ci/test/plan.test.sh bridge/test/server/http.test.ts relay/test/jwt.test.ts ios/FlowKit/Tests/FlowKitTests/GridTests.swift android/core/src/test/kotlin/MessagesTest.kt
expect "the relay"                      false false true  relay/src/index.ts relay/wrangler.jsonc relay/package.json relay/package-lock.json
...
expect "android beside the bridge"      true  false false android/app/src/main/x.kt bridge/src/y.ts
expect "shared fixtures"                true  true false  shared/fixtures/frames/a.json
expect "the delivery workflow"          true  true true  .github/workflows/deliver.yml
expect "a ci/ script"                   true  true true  ci/lane-guard.sh
...
expect "documentation beside code"      false false false docs/OPERATIONS.md bridge/src/config.ts
...
tip HEAD; for l in android ios relay; do deliver "$l" HEAD~1; done
check "an ios file moved to android"     "android=true ios=true relay=false build_number=7"  "$(plan)"
git mv android/app/src/main/Moved.kt docs/Moved.kt && git commit -q -m "android file moved under docs"
tip HEAD; for l in android ios relay; do deliver "$l" HEAD~1; done
check "an android file moved under docs" "android=true ios=false relay=false build_number=7" "$(plan)"
...
tip "$A"
for l in android ios relay; do undeliver "$l"; done
check "never delivered: everything"       "android=true ios=true relay=true build_number=7"   "$(plan)"
for l in android ios relay; do deliver "$l" "$A"; done
check "already delivered (a rerun)"       "android=false ios=false relay=false build_number=7" "$(plan)"
for l in android ios relay; do deliver "$l" "$I"; done
check "every lane delivered at the parent" "android=true ios=false relay=false build_number=7"  "$(plan)"
deliver ios "$X"; deliver android "$I"
check "each lane from its own marker"     "android=true ios=true relay=false build_number=7"   "$(plan)"
deliver ios "$A"; undeliver android
check "one lane never delivered"          "android=true ios=false relay=false build_number=7"  "$(plan)"
undeliver relay
check "the relay never delivered"         "android=true ios=false relay=true build_number=7"   "$(plan)"
deliver relay "$A"
...
check "a lane that failed last time"      "android=false ios=true relay=false build_number=7"  "$(plan)"
...
undeliver ios; deliver android HEAD; deliver relay HEAD
git revert --no-edit HEAD > /dev/null
tip HEAD
check "a revert after a lost marker"      "android=false ios=true relay=false build_number=7"  "$(plan)"
...
git checkout -q "$I"
for l in android ios relay; do deliver "$l" "$X"; done
check "not main's tip"                    "android=false ios=false relay=false build_number=7" "$(plan)"
...
git checkout -q "$branch"
deliver ios "$unrelated"; deliver android "$A"; deliver relay "$A"
check "a marker from another history"     "android=false ios=true relay=false build_number=7"  "$(plan)"
...
deliver ios "$A"
check "a marker this clone does not have" "android=true ios=false relay=false build_number=7"  "$(plan)"
deliver android "$A"
...
check "dispatch: ios only"   "android=false ios=true relay=false build_number=7" "$(plan EVENT=workflow_dispatch IN_IOS=true)"
check "dispatch: relay only" "android=false ios=false relay=true build_number=7" "$(plan EVENT=workflow_dispatch IN_RELAY=true)"
check "dispatch: all"        "android=true ios=true relay=true build_number=7"   "$(plan EVENT=workflow_dispatch IN_ANDROID=true IN_IOS=true IN_RELAY=true)"
check "dispatch: nothing"    "android=false ios=false relay=false build_number=7" "$(plan EVENT=workflow_dispatch)"
